(function () {
  var state = { page: 'home', subpage: 'planos', bootstrap: null, filters: {}, leadFilter: 'todos', newStudentsSort: 'data', followCategory: '', settingsSection: 'alertas', settingsAlertKind: 'prescricoes', settingsDirty: false, settingsAlertDraft: null, settingsHomeDraft: null, profilesExpanded: false, syncQueue: null, syncRetryTimer: null, churnFilters: null, churnCharts: [], churnAnalyticsRequest: 0, churnAnalyticsCache: Object.create(null), backgroundSyncTimer: null };
  ...
  function renderFluxoNovos(items) { var box=section('Alunos novos'), actions=el('div','record-actions'), byDate=el('button',state.newStudentsSort==='data'?'primary':'secondary','Data de entrada'), byName=el('button',state.newStudentsSort==='nome'?'primary':'secondary','A–Z'), list=el('div','lead-list-grid');byDate.type='button';byName.type='button';byDate.addEventListener('click',function(){state.newStudentsSort='data';render();});byName.addEventListener('click',function(){state.newStudentsSort='nome';render();});actions.appendChild(byDate);actions.appendChild(byName);box.appendChild(actions);box.appendChild(el('p','lead-list-summary',number(items.length)+' aluno(s) com entrada oficial registrada.'));items.slice().sort(function(a,b){return state.newStudentsSort==='nome'?String(a.nome).localeCompare(String(b.nome),'pt-BR'):(parseDate(b.dataEntrada)||0)-(parseDate(a.dataEntrada)||0)||String(a.nome).localeCompare(String(b.nome),'pt-BR');}).forEach(function(item){var row=el('article','lead-row'),head=el('div','lead-row-header');head.appendChild(el('h3','',item.nome||'Aluno sem nome'));head.appendChild(el('span','lead-status',item.dataEntrada||'Data não informada'));row.appendChild(head);row.appendChild(el('p','body-copy','ID '+(item.alunoId||'—')+' • '+(item.contrato||'Plano não informado')+' • '+money(item.valor)));list.appendChild(row);});if(!list.childNodes.length)list.appendChild(el('p','body-copy','Nenhum aluno novo importado.'));box.appendChild(list);return[box]; }
  function renderFluxo() { var fluxo=state.bootstrap.fluxo||{};if(state.subpage==='leads')return renderFluxoLeads(fluxo.leads||[]);if(state.subpage==='novos')return renderFluxoNovos(fluxo.novos||[]);return renderFluxoChurns(fluxo.churns||[]); }
  function render() { if(!state.bootstrap)return; var content=document.getElementById('pageContent');if(!(state.page==='fluxo'&&state.subpage==='churns')){state.churnAnalyticsRequest+=1;destruirGraficosChurn();}clear(content);var data=filtered(), nodes=[];document.getElementById('pageTitle').textContent=labels[state.page];document.getElementById('lastUpdate').textContent=state.bootstrap.atualizadoEm?'Base atualizada em '+state.bootstrap.atualizadoEm:'Nenhuma importação válida';var sub=document.getElementById('subnav'), global=document.querySelector('.global-filters');sub.hidden=!(state.page==='financeiro'||state.page==='acompanhamento'||state.page==='fluxo');if(global)global.hidden=state.page==='fluxo';sub.querySelectorAll('button').forEach(function(b){var finance=b.dataset.subpage==='planos'||b.dataset.subpage==='vencimentos'||b.dataset.subpage==='permanencia', follow=b.dataset.subpage==='prescricoes'||b.dataset.subpage==='avaliacoes', flow=b.dataset.subpage==='leads'||b.dataset.subpage==='churns'||b.dataset.subpage==='novos';b.hidden=(state.page==='financeiro')?!finance:(state.page==='acompanhamento')?!follow:(state.page==='fluxo')?!flow:true;b.classList.toggle('active',b.dataset.subpage===state.subpage);});if(state.page==='home')nodes=renderHome(data);else if(state.page==='financeiro')nodes=state.subpage==='planos'?renderPlans(data):state.subpage==='vencimentos'?renderDue(data):renderPermanence(data);else if(state.page==='acompanhamento')nodes=renderFollow(state.subpage);else if(state.page==='fluxo')nodes=renderFluxo();else nodes=renderSettings();nodes.forEach(function(n){content.appendChild(n);});if(state.page==='fluxo'&&state.subpage==='churns')renderChurnAnalytics((state.bootstrap.fluxo||{}).churns||[]); }
  ...
  function activate(page) { if(state.page==='configuracoes'&&page!=='configuracoes'&&state.settingsDirty&&!window.confirm('Descartar alterações não salvas?'))return;state.page=page;if(page==='financeiro'&&['planos','vencimentos','permanencia'].indexOf(state.subpage)<0)state.subpage='planos';if(page==='acompanhamento'&&['prescricoes','avaliacoes'].indexOf(state.subpage)<0)state.subpage='prescricoes';if(page==='fluxo'&&['leads','churns','novos'].indexOf(state.subpage)<0)state.subpage='leads';document.querySelectorAll('[data-page]').forEach(function(b){b.classList.toggle('active',b.dataset.page===page);});render(); }
  ...
  function prepararFilaDeSincronizacao() {
    ...
    state.syncQueue.subscribe(function(info){if(info.status==='saving'){setSave('Salvando…');setRetryVisible(false);}else if(info.status==='pending'){setSave(info.error?'Alterações pendentes. Tentaremos novamente.':'Alteração pendente');setRetryVisible(true);agendarReenvioDaFila();}else{setSave('Salvo');setRetryVisible(false);}});
    state.syncQueue.status().then(function(){return state.syncQueue.flush();}).then(function(sent){if(!sent)agendarReenvioDaFila();});
  }
  function enqueue(patch) { aplicarMutacaoOtimista(patch);return state.syncQueue.enqueue([patch]).then(function(){return state.syncQueue.flush();}); }
  function tentarNovamenteMutacoes() { if(state.syncQueue)state.syncQueue.retryNow().then(function(sent){if(!sent)agendarReenvioDaFila();}); }
  function start() { prepararFilaDeSincronizacao();setProgress(8);var timer=setInterval(function(){var width=parseFloat(document.getElementById('loading-progress').style.width)||8;setProgress(Math.min(95,width+7));},150);setTimeout(function(){if(!state.bootstrap)document.getElementById('loading-message').textContent='A base está levando mais tempo que o normal…';},5000);var cached=safeCacheGet();if(cached&&cached.versao){applyBootstrap(cached);call('obterVersaoDashboard').then(function(v){if(v.versao!==cached.versao)return call('obterBootstrapDashboard');}).then(function(fresh){if(fresh)applyBootstrap(fresh);setSave('Base sincronizada');}).catch(function(){setSave('Sem conexão com a base. Exibindo dados salvos neste dispositivo.');}).finally(function(){clearInterval(timer);});}else{call('obterBootstrapDashboard').then(function(fresh){applyBootstrap(fresh);setSave('Base sincronizada');}).catch(function(){document.getElementById('loading-message').textContent='Não foi possível conectar à base. Verifique a conexão e tente novamente.';}).finally(function(){clearInterval(timer);});} }
  window.iniciarDashboardPwa=function(){ document.querySelectorAll('[data-page]').forEach(function(b){b.addEventListener('click',function(){activate(b.dataset.page);});});document.getElementById('subnav').addEventListener('click',function(e){if(e.target.dataset.subpage){state.subpage=e.target.dataset.subpage;if(state.page==='acompanhamento')state.followCategory='';render();}});document.getElementById('globalStatus').addEventListener('change',function(e){state.filters.status=e.target.value;render();});document.getElementById('globalPolo').addEventListener('change',function(e){state.filters.polo=e.target.value;render();});document.getElementById('detailClose').addEventListener('click',function(){document.getElementById('detailDialog').close();});var retry=document.getElementById('retryMutations');if(retry)retry.addEventListener('click',tentarNovamenteMutacoes);window.addEventListener('online',function(){if(state.syncQueue)state.syncQueue.retryNow();});window.addEventListener('beforeunload',function(event){if(!state.settingsDirty)return;event.preventDefault();event.returnValue='';});start(); };
}());
